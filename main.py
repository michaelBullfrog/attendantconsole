...
def get_token_for_dnd(target_person_id: str) -> str:
    """Use the Webex admin token for DND updates/reads.

    DND changes should behave like Control Hub/admin changes. Using the
    signed-in user's OAuth token can return 403 for users who are otherwise
    valid Webex Calling users.
    """
    if WEBEX_ADMIN_TOKEN:
        return WEBEX_ADMIN_TOKEN.strip()

    raise HTTPException(
        status_code=500,
        detail="WEBEX_ADMIN_TOKEN is required for DND controls. Add it in Render and redeploy.",
    )


def dnd_endpoint_candidates(person_id: str) -> list[str]:
    """Return only the public Webex DND endpoint.

    Do not try the old hydra/telephony/config fallback because it returns
    misleading 404 errors and can hide the real Webex API response.
    """
    return [WEBEX_DND_ENDPOINT_TEMPLATE.format(person_id=person_id)]
...
def set_person_dnd_settings(person_id: str, org_id: Optional[str], enabled: bool, ring_reminder: Optional[bool] = None) -> Dict[str, Any]:
    ...
    for url in dnd_endpoint_candidates(person_id):
        ...
        for body in payloads:
            ...
            attempts.append({
                ...
                "used_admin_token": bool(WEBEX_ADMIN_TOKEN),
            })
            if response.status_code in (200, 201, 202, 204):
                data = response.json() if response.text else {}
                with db() as conn:
    ...
    raise HTTPException(status_code=502, detail=f"Unable to update DND settings in Webex. Attempts: {json.dumps(attempts)}")
