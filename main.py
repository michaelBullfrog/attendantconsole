...
function renderDndControl(agent) {
  ...
  const isOn = dnd === true;
  const label = isOn ? "DND On" : (dnd === false ? "DND Off" : "DND Unknown");
  const pillClass = isOn ? "DndOn" : "DndOff";
  ...
  const btnClass = isOn ? "off" : "on";
  const disabled = !agent.person_id || !agent.authenticated;
  const title = disabled ? "DND unavailable until this user authenticates" : `${btnText} Do Not Disturb`;
  return `<div class="dnd-actions"><span class="pill ${pillClass}">${label}</span><button class="dnd-btn ${btnClass}" ${disabled ? "disabled" : ""} title="${title}" onclick='setDnd(${personId}, ${nextEnabled}, ${userLabel})'>${btnText}</button></div>`;
}

async function setDnd(personId, enabled, userLabel) {
  if (!personId) return;
  const action = enabled ? "turn ON" : "turn OFF";
  if (!confirm(`Are you sure you want to ${action} Do Not Disturb for ${userLabel}?`)) return;
