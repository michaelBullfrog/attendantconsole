...
def revio_auth():
    if REVIO_PSA_AUTH_HEADER or REVIO_PSA_BEARER_TOKEN:
        return None
...
def revio_is_configured() -> bool:
    has_auth = bool(REVIO_PSA_AUTH_HEADER or REVIO_PSA_BEARER_TOKEN or (REVIO_PSA_BASIC_USERNAME and REVIO_PSA_BASIC_PASSWORD))
...
def send_revio_ticket_for_call_log(call_log_id: int) -> Dict[str, Any]:
    ...
    url = f"{REVIO_PSA_BASE_URL}{REVIO_PSA_TICKET_ENDPOINT}"
    ...
    error_text = f"{response.status_code}: {response.text[:1000]}"
    with db() as conn:
        conn.execute(
            """
            UPDATE call_logs
...
async function sendCallLogToPsa(callLogId) {
  ...
  try {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok || !data.success) {
      showTransferStatus(data.detail || data.message || "Rev.io ticket creation failed. Check Render logs and Rev.io settings.", "error");
      await loadCallLogs();
      return;
    }
