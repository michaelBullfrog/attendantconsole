...
def send_zoho_record_for_call_log(call_log_id: int, direction_override: Optional[str] = None) -> Dict[str, Any]:
      ...
      <div class="call-log-item">
        ...
        <div>
          ...
          <div class="zoho-actions">
            <button class="small zoho-button" ${zohoDisabled ? "disabled" : ""} title="Send to Zoho using the detected or current direction shown on this row" onclick="sendCallLogToZoho(${log.id})">Zoho Inbound</button>
            <button class="small zoho-button outbound" ${zohoDisabled ? "disabled" : ""} title="Send to Zoho and force this call to Outbound" onclick="sendCallLogToZoho(${log.id}, 'Outbound')">Zoho Outbound</button>
          </div>
        </div>
      </div>
    `;
...
async function sendCallLogToZoho(callLogId, directionOverride = null) {
  const directionText = directionOverride ? ` as ${directionOverride}` : " as Inbound/Detected";
  ...
  try {
    const res = await fetch(`/api/call-logs/${encodeURIComponent(callLogId)}/send-zoho`, {
      ...
      headers: { "Content-Type": "application/json" },
      credentials: "same-origin",
      body: JSON.stringify(directionOverride ? { direction: directionOverride } : {})
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || !data.success) {
...
async def api_send_call_log_to_zoho(call_log_id: int, request: Request):
    ...
    result = send_zoho_record_for_call_log(call_log_id, direction_override=direction_override)
    return {"success": True, "call_log_id": call_log_id, **result}
