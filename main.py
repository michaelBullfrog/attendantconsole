  ...
  list.innerHTML = recentCallLogs.map(log => {
    ...
    return `
      <div class="call-log-item">
        ...
        <div title="${remote}">${remote}<br><span class="activity-meta">Zoho Direction: ${direction}</span></div>
        ...
        <div>
          ...
          <button class="small zoho-button" ${zohoDisabled ? "disabled" : ""} onclick="sendCallLogToZoho(${log.id}, 'Outbound')">Send Zoho Outbound</button>
        </div>
      </div>
    `;
  }).join("");
