...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .dnd-select.on { background: #fee2e2; color: #991b1b; border-color: #fecaca; }
    .dnd-select.off { background: #dcfce7; color: #166534; border-color: #bbf7d0; }
    .dnd-select.unknown { background: #e5e7eb; color: #374151; }
    .dnd-select:disabled { background: #e5e7eb; color: #9ca3af; cursor: not-allowed; }
...
const DEFAULT_COLUMNS = [
  ...
  { key: "duration", label: "Time in State", render: a => `<span class="duration">${formatStateDuration(a)}</span>` },
...
function renderDndControl(agent) {
  ...
  const userLabel = JSON.stringify(agent.email || agent.display_name || "this user");
  const disabled = !agent.person_id || !agent.authenticated;
  const selectClass = isOn ? "on" : (isOff ? "off" : "unknown");
  const title = disabled ? "DND unavailable until this user authenticates" : "Change Do Not Disturb";

  return `<div class="dnd-actions">
    <select class="dnd-select ${selectClass}" ${disabled ? "disabled" : ""} title="${title}" onchange='setDnd(${personId}, this.value === "on", ${userLabel}, this)'>
      <option value="" ${dnd === null ? "selected" : ""}>DND Unknown</option>
      <option value="off" ${isOff ? "selected" : ""}>DND Off</option>
      <option value="on" ${isOn ? "selected" : ""}>DND On</option>
    </select>
  </div>`;
}

async function setDnd(personId, enabled, userLabel, selectEl = null) {
  if (!personId) return;
  if (selectEl && selectEl.value === "") return;

  const action = enabled ? "turn ON" : "turn OFF";
  if (!confirm(`Are you sure you want to ${action} Do Not Disturb for ${userLabel}?`)) {
    await loadAgents();
    return;
  }
  ...
  try {
    ...
    if (!res.ok || !data.success) {
      showTransferStatus(data.detail || data.message || "DND update failed. Check Render logs and Webex admin token permissions.", "error");
      await loadAgents();
      return;
    }
...
// Poll less aggressively so several open browser tabs do not hammer a small Render service.
// Timers still update locally, just not every second.
    ...
    <html>
      ...
      <body>
        <div class="card">
          ...
          <p class="muted">This browser will stay signed in for call control for up to 90 days, as long as the Webex refresh token remains valid.</p>
