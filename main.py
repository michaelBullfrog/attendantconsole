...
# If omitted, the app still works but location may show as Unknown Organization.
...
def ensure_org_columns():
    with db() as conn:
        ...
        if "org_id" not in columns:
            conn.execute("ALTER TABLE agents ADD COLUMN org_id TEXT")
        if "org_name" not in columns:
            conn.execute("ALTER TABLE agents ADD COLUMN org_name TEXT")
    ...
    for url in possible_urls:
        try:
            ...
            if resp.status_code >= 400:
                print(f"Organization lookup skipped for {url}: {resp.status_code} {resp.text}")
                continue
            ...
            print(f"Organization lookup error for {url}: {exc}")
    ...
    if location_id and not location_name:
        ...
        try:
            ...
            if resp.status_code < 400:
                ...
                print(f"Organization name lookup skipped: {resp.status_code} {resp.text}")
        except Exception as exc:
            print(f"Organization name lookup error: {exc}")
...
def upsert_agent_from_oauth(me: Dict[str, Any], webhook: Dict[str, Any], user_access_token: str):
    ...
    location_name = location.get("location_name") or "Unknown Organization"
...
def sync_agent_org(person_id: str, org_id: Optional[str], org_name: Optional[str]):
    if not person_id:
        return
    with db() as conn:
        conn.execute(
            "UPDATE agents SET org_id = COALESCE(?, org_id), org_name = COALESCE(?, org_name) WHERE person_id = ?",
            (org_id, org_name, person_id),
        )
...
def resolve_org_name(org_id: Optional[str], user_access_token: Optional[str] = None) -> str:
    """
    Converts the long Webex orgId into a readable organization name.

    Best option:
      WEBEX_ADMIN_TOKEN with spark-admin:organizations_read or partner/admin access.

    Fallback:
      user OAuth token, if it has access.

    Final fallback:
      return the orgId so nothing breaks.
    """
    if not org_id:
        return "Unknown Org"
    ...
    for token in tokens_to_try:
        try:
            ...
            if response.status_code < 400:
                data = response.json()
                name = data.get("displayName") or data.get("name") or org_id
...
<header>
  ...
  <p>Monitor users by organization, state, and duration.</p>
</header>
...
<main>
  ...
  <div class="note">
    Org names are resolved using WEBEX_ADMIN_TOKEN when available. If the app cannot resolve the org, it will show the original org ID.
  </div>
</main>
...
def api_agents():
    ...
    for a in agents:
        a["org_name"] = a.get("org_name") or a.get("org_id") or "Unknown Org"
