...
# By default this app now treats REVIO_PSA_API_KEY as a Bearer token to match
# that working bot behavior.
REVIO_PSA_API_KEY = os.getenv("REVIO_PSA_API_KEY", "")
REVIO_PSA_HOST = os.getenv("REVIO_PSA_HOST", "")
REVIO_PSA_API_KEY_AUTH_MODE = os.getenv("REVIO_PSA_API_KEY_AUTH_MODE", "bearer").strip().lower()
...
def revio_headers() -> Dict[str, str]:
    ...
    # Mirror the working bot variables. Some Rev.io PSA tenants use the tenant
    # host for routing even when the URL is https://api.psarev.io.
    if REVIO_PSA_HOST:
        headers["Host"] = REVIO_PSA_HOST.strip()
    ...
    if REVIO_PSA_BEARER_TOKEN:
        ...
        if REVIO_PSA_API_KEY_AUTH_MODE in {"bearer", "token"}:
...
def get_revio_auth_debug() -> Dict[str, Any]:
    ...
    if REVIO_PSA_API_KEY and not authorization_attached:
        authorization_attached = REVIO_PSA_API_KEY_AUTH_MODE in {"bearer", "token"} or (REVIO_PSA_API_KEY_HEADER or "Authorization").strip().lower() == "authorization"
    ...
    return {
        ...
        "host_header_attached": bool(REVIO_PSA_HOST),
...
def api_revio_debug():
    ...
    return {
        ...
        "notes": [
            ...
            "By default, REVIO_PSA_API_KEY is sent as Authorization: Bearer <key>, and REVIO_PSA_HOST is sent as the Host header when configured.",
            "If status is 401 and auth_header_attached is true, compare this output to your working bot get_psa_headers() output.",
        ],
    }
