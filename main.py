...
def extract_work_extension_from_person(person_payload: Dict[str, Any]) -> Optional[str]:
    """
    Pulls extension from GET /v1/people/{personId} response.
...
def resolve_user_extension(person_id: Optional[str], user_access_token: Optional[str] = None) -> Optional[str]:
    """
    Best-effort lookup for the user's extension using:
      GET https://webexapis.com/v1/people/{personId}

    Token order:
      1. WEBEX_ADMIN_TOKEN, if set
      2. User OAuth token from the connect/disconnect flow

    It returns the phoneNumbers value where type == work_extension.
    """
    if not person_id:
        return None
    ...
    for token in tokens_to_try:
        try:
            ...
            if response.status_code < 400:
                extension = extract_work_extension_from_person(response.json())
                if extension:
                    return extension

                print(f"People lookup returned no work_extension for {person_id}: {response.text}")
            else:
                print(f"People lookup failed for {person_id}: {response.status_code} {response.text}")

        except Exception as exc:
            print(f"People extension lookup exception for {person_id}: {exc}")
...
const DEFAULT_COLUMNS = [
  ...
  { key: "action", label: "Action", render: a => `<button class="secondary small" onclick="removeAgent('${a.person_id}')">Remove Row</button>` }
];
...
async function loadAgents() {
  try {
    ...
    document.getElementById("agentBody").innerHTML =
      `<tr><td class="empty">Could not load /api/agents. Check Render logs.</td></tr>`;
  }
}
...
function renderTable() {
  ...
  body.innerHTML = filtered.map(a => `
    <tr>
      ${columns.map(c => `<td>${c.render(a)}</td>`).join("")}
    </tr>
  `).join("");
}
...
def build_webex_authorize_url(redirect_uri: str, state: str) -> str:
    """
    Builds a properly URL-encoded Webex OAuth authorize URL.
    """
    params = {
...
def oauth_config_error_html(missing_items):
    missing = ", ".join(missing_items)
    return HTMLResponse(f"""
    <html>
      <head>
        <title>Webex OAuth Configuration Error</title>
        <style>
          body {{ font-family: Arial, sans-serif; background: #eef3f8; padding: 40px; color: #101828; }}
          .card {{ background: white; padding: 24px; border-radius: 16px; max-width: 760px; box-shadow: 0 8px 18px rgba(15,23,42,.08); }}
      ...
      <body>
        <div class="card">
          ...
          <p>The app is missing this Render environment variable:</p>
          <p><code>{missing}</code></p>
          <p>Add it in Render, redeploy, then try <code>/oauth/start</code> again.</p>
        </div>
      </body>
    </html>
    """, status_code=500)
...
def oauth_remove_start():
    ...
    if not WEBEX_CLIENT_ID:
        missing.append("WEBEX_CLIENT_ID")
    if not WEBEX_REDIRECT_URI:
        missing.append("WEBEX_REDIRECT_URI")

    if missing:
        return oauth_config_error_html(missing)
...
def oauth_callback(request: Request):
    ...
    if not code:
        ...
        <html>
          <head>
            <title>Webex Authorization Failed</title>
            <style>
              body {{ font-family: Arial, sans-serif; background: #eef3f8; padding: 40px; color: #101828; }}
              .card {{ background: white; padding: 24px; border-radius: 16px; max-width: 760px; box-shadow: 0 8px 18px rgba(15,23,42,.08); }}
              .error {{ color: #991b1b; font-weight: 800; }}
          ...
          <body>
            <div class="card">
              <h2 class="error">Webex Authorization Failed</h2>
              <p>Webex did not return an authorization code.</p>
              <p><strong>Error:</strong> <code>{error or "N/A"}</code></p>
              <p><strong>Description:</strong> <code>{error_description or "N/A"}</code></p>
              <p>Start over from <code>/oauth/start</code>.</p>
            </div>
          </body>
        </html>
        """, status_code=400)
