...
def get_webex_meeting_details(access_token: str, meeting_id: str) -> Dict[str, Any]:
    ...
    if response.status_code >= 400:
        raise HTTPException(status_code=response.status_code, detail=f"Webex meeting details failed: {response.text}")
...
<main>
  ...
  <section class="activity-panel">
    ...
    <div class="meeting-inline-send">
      <input id="inlineMeetingId" type="text" placeholder="Paste Webex meeting ID to send to Rev.io..." />
...
async function sendExistingMeetingToRev(meetingId) {
  ...
  const input = document.getElementById("inlineMeetingId");
  if (input) input.value = meetingId;
...
async def api_send_meeting_to_rev(request: Request):
    ...
    result = send_revio_ticket_for_meeting(meeting_id, current_person_id, force=force)
    return result
