...
# Rev.io PSA API key support. For testing, set REVIO_PSA_API_KEY and leave
# the header/scheme defaults unless Rev.io tells you otherwise.
# Default sends: Authorization: rev <api_key>
REVIO_PSA_API_KEY = os.getenv("REVIO_PSA_API_KEY", "")
REVIO_PSA_API_KEY_HEADER = os.getenv("REVIO_PSA_API_KEY_HEADER", "Authorization")
REVIO_PSA_API_KEY_SCHEME = os.getenv("REVIO_PSA_API_KEY_SCHEME", "rev")
REVIO_PSA_BASIC_USERNAME = os.getenv("REVIO_PSA_BASIC_USERNAME", "")
REVIO_PSA_BASIC_PASSWORD = os.getenv("REVIO_PSA_BASIC_PASSWORD", "")
REVIO_TICKET_TYPE_ID = int(os.getenv("REVIO_TICKET_TYPE_ID", "0"))
REVIO_TICKET_STATUS_ID = int(os.getenv("REVIO_TICKET_STATUS_ID", "0"))
REVIO_TICKET_PRIORITY_ID = int(os.getenv("REVIO_TICKET_PRIORITY_ID", "0"))
...
def revio_headers() -> Dict[str, str]:
    ...
    # Prefer API key auth when configured so testing can override an older Basic
    # Authorization header left in Render environment variables.
    if REVIO_PSA_API_KEY:
        ...
        headers["Authorization"] = REVIO_PSA_AUTH_HEADER
    elif REVIO_PSA_BEARER_TOKEN:
        headers["Authorization"] = f"Bearer {REVIO_PSA_BEARER_TOKEN}"
    return headers
...
def get_revio_auth_debug() -> Dict[str, Any]:
    ...
    if REVIO_PSA_API_KEY:
        ...
        api_key_header_name = (REVIO_PSA_API_KEY_HEADER or "Authorization").strip()
    ...
    authorization_attached = bool(REVIO_PSA_API_KEY or REVIO_PSA_AUTH_HEADER or REVIO_PSA_BEARER_TOKEN)
    if REVIO_PSA_API_KEY and (REVIO_PSA_API_KEY_HEADER or "Authorization").strip().lower() != "authorization":
...
def create_call_log_if_completed(conn: sqlite3.Connection, existing: sqlite3.Row, ended_at: str) -> Optional[int]:
...
def cleanup_unauthenticated_agents(conn: sqlite3.Connection) -> int:
...
def init_db():
    with db() as conn:
        ...
        for column_name in ["psa_ticket_status", "psa_ticket_id", "psa_error"]:
...
def api_revio_debug():
    ...
    return {
        ...
        "notes": [
            ...
            "If status is 401 and auth_header_attached is true, confirm the credentials are valid for the same Rev.io PSA API environment.",
        ],
    }
...
<main>
  ...
  <section class="activity-panel">
    <div class="activity-header">
      <h2>Completed Calls / PSA Ticket Log</h2>
      <button class="secondary small" onclick="loadCallLogs()">Refresh PSA Log</button>
    </div>
...
function renderCallLogs() {
  ...
  list.innerHTML = recentCallLogs.map(log => {
    ...
    const statusLabel = status === "sent" ? `Sent${log.psa_ticket_id ? ` #${log.psa_ticket_id}` : ""}` : (status === "failed" ? "Failed" : "Not sent");
    return `
