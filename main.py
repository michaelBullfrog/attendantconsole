...
# Optional: use an admin/partner token to enrich users with Webex Calling location names.
# If omitted, the app still works but location may show as Unknown Location.
...
def classify_status(event: Dict[str, Any]) -> str:
    """
    Webex Calling telephony_calls mapping:
...
def extract_person_id(event: Dict[str, Any]) -> str:
    """
    In Webex Calling telephony_calls payloads, actorId / createdBy typically identify the
    Webex user whose OAuth token owns the webhook.
    """
    data = event.get("data", {}) if isinstance(event.get("data"), dict) else {}
...
def get_nested_value(obj: Any, keys: set) -> Optional[str]:
    """Recursively search a nested Webex response for a useful field."""
    if isinstance(obj, dict):
        for key, value in obj.items():
            if key in keys and value:
                return str(value)
        for value in obj.values():
            found = get_nested_value(value, keys)
            if found:
                return found
    elif isinstance(obj, list):
        for item in obj:
            found = get_nested_value(item, keys)
            if found:
                return found
    return None


def lookup_calling_location(user_access_token: str, person_id: str, org_id: Optional[str]) -> Dict[str, Optional[str]]:
    """
    Best-effort Webex Calling location lookup.

    This app's normal OAuth user token may not have enough admin rights to read
    Webex Calling configuration. If you add WEBEX_ADMIN_TOKEN in Render, the app
    will use that token first to try to enrich the user with locationId/locationName.
    """
    token = WEBEX_ADMIN_TOKEN or user_access_token
    if not token or not person_id:
        return {"location_id": None, "location_name": None}

    headers = {"Authorization": f"Bearer {token}"}
    query = f"?orgId={org_id}" if org_id else ""

    # Try the common Webex Calling person config endpoint first.
    possible_urls = [
        f"https://webexapis.com/v1/telephony/config/people/{person_id}{query}",
    ...
    for url in possible_urls:
        try:
            ...
            if resp.status_code >= 400:
                print(f"Location lookup skipped for {url}: {resp.status_code} {resp.text}")
                continue

            payload = resp.json()
            location_id = location_id or get_nested_value(payload, {"locationId", "location_id"})
            location_name = location_name or get_nested_value(payload, {"locationName", "location_name"})

            # Some responses may include location as an object with a name/id.
            location_obj = payload.get("location") if isinstance(payload, dict) else None
            if isinstance(location_obj, dict):
                location_id = location_id or location_obj.get("id")
                location_name = location_name or location_obj.get("name")

            if location_id or location_name:
                break
        except Exception as exc:
            print(f"Location lookup error for {url}: {exc}")

    # If we found an ID but not a name, try the location detail endpoint.
    if location_id and not location_name:
        loc_query = f"?orgId={org_id}" if org_id else ""
        loc_url = f"https://webexapis.com/v1/telephony/config/locations/{location_id}{loc_query}"
        try:
            resp = requests.get(loc_url, headers=headers, timeout=20)
            if resp.status_code < 400:
                payload = resp.json()
                location_name = payload.get("name") or payload.get("locationName") or location_name
            else:
                print(f"Location name lookup skipped: {resp.status_code} {resp.text}")
        except Exception as exc:
            print(f"Location name lookup error: {exc}")
...
def create_call_status_webhook(user_access_token: str) -> Dict[str, Any]:
    """
    Creates a user-level Webex Calling telephony_calls webhook.

    Important:
    Webex returns 409 Conflict if the same user already has a webhook with the same
    resource/event/targetUrl filter. This function first checks existing webhooks and
    ...
    def find_existing_webhook() -> Optional[Dict[str, Any]]:
        ...
        for webhook in existing_webhooks:
            same_resource = webhook.get("resource") == "telephony_calls"
            same_target = webhook.get("targetUrl") == WEBEX_WEBHOOK_TARGET_URL

            # event can be "all" or separate event types if you created older test hooks
            compatible_event = webhook.get("event") in {
                "all",
                "created",
                "updated",
                "deleted",
            ...
            if same_resource and same_target and compatible_event:
    ...
    # If Webex still reports duplicate, retry listing and reuse the duplicate.
...
def upsert_agent_from_oauth(me: Dict[str, Any], webhook: Dict[str, Any], user_access_token: str):
    ...
    org_id = me.get("orgId")
    org_name = me.get("orgName") or org_id

    if not person_id:
        return

    location = lookup_calling_location(user_access_token, person_id, org_id)
    location_id = location.get("location_id")
    location_name = location.get("location_name") or "Unknown Location"
    ...
    with db() as conn:
        ...
        if existing:
            conn.execute("""
                ...
                WHERE person_id = ?
            """, (email, display_name, org_id, org_name, location_id, location_name, webhook.get("id"), ts, person_id))
        else:
            conn.execute("""
                INSERT INTO agents (
                    person_id, email, display_name, org_id, org_name, location_id, location_name,
                    status, state_started_at, updated_at, webhook_id
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, 'Not On Call', ?, ?, ?)
            """, (person_id, email, display_name, org_id, org_name, location_id, location_name, ts, ts, webhook.get("id")))
...
def update_agent_from_event(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with db() as conn:
        ...
        if existing:
            ...
            conn.execute("""
                UPDATE agents
                SET status = ?, org_id = COALESCE(?, org_id), webex_state = ?, event_type = ?,
                    call_id = ?, call_session_id = ?, remote_name = ?, remote_number = ?,
                    remote_call_type = ?, state_started_at = ?, updated_at = ?, webhook_id = ?
                WHERE person_id = ?
            """, (
                new_status,
                org_id,
            ...
            conn.execute("""
                INSERT INTO agents (
                    person_id, email, display_name, org_id, org_name, location_id, location_name,
                    status, webex_state, event_type, call_id, call_session_id, remote_name,
                    remote_number, remote_call_type, state_started_at, updated_at, webhook_id
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                person_id,
...
<header>
  ...
  <p>Monitor connected Webex Calling users by current state, location, organization, and duration.</p>
</header>

<main>
  <div class="toolbar">
    <div class="toolbar-left">
      <input id="search" placeholder="Search email, name, number, location..." oninput="renderTable()" />
  ...
  <div class="note">
    Dashboard refreshes every 3 seconds. Duration updates every second.
  </div>
</main>
...
function uniqueSorted(values) {
  return [...new Set(values.filter(v => v && v !== "None"))].sort((a, b) => a.localeCompare(b));
}
...
function updateFilters() {
  const orgs = uniqueSorted(agents.map(a => a.org_name || a.org_id || "Unknown Organization"));
  const locations = uniqueSorted(agents.map(a => a.location_name || "Unknown Location"));
  setSelectOptions("orgFilter", orgs, "All Organizations");
  setSelectOptions("locationFilter", locations, "All Locations");
}
...
function renderTable() {
  ...
  const filtered = agents.filter(a => {
    const orgValue = a.org_name || a.org_id || "Unknown Organization";
    const locationValue = a.location_name || "Unknown Location";
    const matchesState = filter === "All" || a.status === filter;
    const matchesOrg = orgFilter === "All" || orgValue === orgFilter;
    const matchesLocation = locationFilter === "All" || locationValue === locationFilter;
    const blob = JSON.stringify(a).toLowerCase();
    return matchesState && matchesOrg && matchesLocation && blob.includes(search);
  });

  const body = document.getElementById("agentBody");

  if (!filtered.length) {
    body.innerHTML = `<tr><td colspan="11" class="empty">No agents found. Click Connect Webex User, authorize a user, then make or receive a test call.</td></tr>`;
    return;
  }
  ...
  body.innerHTML = filtered.map(a => `
    <tr>
      ...
      <td>
        <div>${a.location_name || "Unknown Location"}</div>
        <div class="subtext">${a.location_id || ""}</div>
      </td>
      <td>
        <div>${a.org_name || a.org_id || "Unknown Organization"}</div>
        <div class="subtext">${a.org_id || ""}</div>
      </td>
...
def api_agents():
    ...
    agents.sort(key=lambda a: (a.get("org_name") or a.get("org_id") or "", a.get("location_name") or "", priority.get(a.get("status"), 9), a.get("email") or a.get("display_name") or ""))
