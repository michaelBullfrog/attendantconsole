...
ZOHO_TIME_SEND_MODE = os.getenv("ZOHO_TIME_SEND_MODE", "display_wall_clock").strip().lower() or "display_wall_clock"
...
def send_revio_ticket_for_call_log(call_log_id: int) -> Dict[str, Any]:
        ...
        if ZoneInfo:
            ...
            if ZOHO_TIME_SEND_MODE in {"display", "display_wall_clock", "wall_clock", "compensate"}:
                try:
                    ...
                    # Preserve the intended local wall-clock time, but attach the
                    # timezone Zoho uses for display. Zoho will convert/display it
                    # back to the same wall-clock time in that CRM timezone.
...
def format_zoho_call_duration(duration_seconds: int) -> str:
    """Return Zoho Calls duration in HH:mm format.

    Zoho's standard Calls module defines Call_Duration as HH:mm. That field
    cannot represent seconds precisely, and Zoho does not allow zero duration
    for completed calls. We therefore round up to at least one minute for the
    standard field and store exact seconds in Description and, optionally, a
    custom field configured with ZOHO_EXACT_DURATION_FIELD_API_NAME.
    """
...
def build_zoho_call_payload(call_log: Dict[str, Any]) -> Dict[str, Any]:
    ...
    description = (
        ...
        f"Zoho Standard Call_Duration: {format_zoho_call_duration(duration_seconds)} (Zoho stores this field as HH:mm)\n"
    ...
    record = {
        "Subject": subject,
        "Call_Type": "Outbound" if (call_log.get("remote_call_type") or "").lower() == "outbound" else "Inbound",
        "Call_Start_Time": format_zoho_datetime(call_log.get("started_at")),
...
def send_zoho_record_for_call_log(call_log_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # If an access token is expired and we have refresh credentials, refresh and retry once.
    ...
    if ok:
        ...
        return {"zoho_record_id": record_id, "zoho_response": response_payload}

    error_text = f"{response.status_code}: {json.dumps(response_payload)[:1500]}"
    if record_message:
        error_text = f"{response.status_code}: {record_message} | {json.dumps(response_payload)[:1500]}"
    with db() as conn:
        conn.execute(
            "UPDATE call_logs SET zoho_status = 'failed', zoho_error = ?, updated_at = ? WHERE id = ?",
            (error_text, ts, call_log_id),
...
def create_call_log_if_completed(conn: Any, existing: Any, ended_at: str) -> Optional[int]:
    ...
    insert_sql = """
        INSERT INTO call_logs (
            ...
            remote_name, remote_number, remote_call_type, started_at, ended_at,
    ...
    cursor = conn.execute(
        ...
        (
            ...
            existing["remote_name"], existing["remote_number"], existing["remote_call_type"],
    ...
    with db() as conn:
        ...
        for table, columns in {
            "agents": [
                ...
                "refresh_token TEXT", "token_expires_at TEXT", "dnd_enabled TEXT", "dnd_ring_reminder TEXT",
            ],
            "events": ["org_id TEXT", "org_name TEXT"],
            "call_logs": ["psa_ticket_status TEXT", "psa_ticket_id TEXT", "psa_error TEXT", "zoho_status TEXT", "zoho_record_id TEXT", "zoho_error TEXT"],
        }.items():
...
def init_sqlite_db():
    with db() as conn:
        ...
        for column_name in ["psa_ticket_status", "psa_ticket_id", "psa_error", "zoho_status", "zoho_record_id", "zoho_error"]:
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .zoho-button { background: #7c3aed; }
    .zoho-button:disabled { background: #e5e7eb; color: #9ca3af; cursor: not-allowed; }
    .org-row td {
      background: #ecfdf3;
      color: #14532d;
      font-weight: 900;
    ...
    return `
      <div class="call-log-item">
        ...
        <div>
          ...
          <button class="small" ${disabled ? "disabled" : ""} onclick="sendCallLogToPsa(${log.id})">Send PSA</button>
          <div class="psa-status ${zohoStatus}" style="margin-top:6px;">${zohoLabel}</div>
          ${zohoStatus === "failed" && log.zoho_error ? `<div class="activity-meta" title="${log.zoho_error}">${log.zoho_error}</div>` : ""}
          <button class="small zoho-button" ${zohoDisabled ? "disabled" : ""} onclick="sendCallLogToZoho(${log.id})">Send Zoho</button>
        </div>
      </div>
    `;
  }).join("");
...
async function sendCallLogToPsa(callLogId) {
...
async function sendCallLogToZoho(callLogId) {
  showTransferStatus("Sending completed call information to Zoho CRM...", "info");
  try {
...
def api_send_call_log_to_psa(call_log_id: int):
    result = send_revio_ticket_for_call_log(call_log_id)
    return {"success": True, "call_log_id": call_log_id, **result}


@app.post("/api/call-logs/{call_log_id}/send-zoho")
def api_send_call_log_to_zoho(call_log_id: int):
    result = send_zoho_record_for_call_log(call_log_id)
    return {"success": True, "call_log_id": call_log_id, **result}
