...
def infer_call_direction_from_event(
    ...
    # Check common explicit direction-style fields first.
    candidate_values = [
        data.get("direction"),
    ...
    for candidate in candidate_values:
        normalized = normalize_call_direction(candidate)
        if normalized:
            return normalized
    ...
    if event_type in {"initiated", "originated", "dialing", "outgoing"} or state in {"dialing", "originating"}:
        return "Outbound"
...
def init_postgres_db():
    ...
    with db() as conn:
        ...
        for table, columns in {
            ...
            "call_logs": ["psa_ticket_status TEXT", "psa_ticket_id TEXT", "psa_error TEXT", "zoho_status TEXT", "zoho_record_id TEXT", "zoho_error TEXT", "call_direction TEXT"],
        }.items():
...
def init_sqlite_db():
    with db() as conn:
        ...
        for column_name in ["psa_ticket_status", "psa_ticket_id", "psa_error", "zoho_status", "zoho_record_id", "zoho_error", "call_direction"]:
