...
function renderDndControl(agent) {
  ...
  return `<div class="dnd-actions">
    <select class="dnd-select ${selectClass}" ${disabled ? "disabled" : ""} title="${title}" data-person-id="${personId}" data-user-label="${userLabel}">
...
async function setDnd(personId, enabled, userLabel, selectEl = null) {
  ...
  const previousValue = selectEl ? selectEl.getAttribute("data-previous-value") || "" : "";
...
function attachDndDropdownHandler() {
  document.addEventListener("focusin", event => {
    const select = event.target.closest && event.target.closest(".dnd-select");
    if (select) select.setAttribute("data-previous-value", select.value || "");
  });
  ...
  document.addEventListener("change", event => {
    ...
    const userLabel = select.getAttribute("data-user-label") || "this user";
    if (!select.value) return;
    setDnd(personId, select.value === "on", userLabel, select);
  });
}

attachDndDropdownHandler();
loadAgents();
loadEvents();
