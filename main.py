...
SCOPES = "spark:calls_read spark:calls_write spark:webhooks_write spark:webhooks_read spark:people_read spark-admin:organizations_read spark-admin:people_read meeting:schedules_read meeting:participants_read meeting:transcripts_read meeting:summaries_read"
...
# Webex Meetings -> Rev.io PSA settings. Uses the same per-user Webex OAuth token
# stored in the agents table by /oauth/start. No shared WEBEX_MEETINGS_TOKEN is used.
MEETING_INTERNAL_DOMAINS = [
    domain.strip().lower()
    for domain in os.getenv("MEETING_INTERNAL_DOMAINS", os.getenv("INTERNAL_DOMAINS", "bullfrog.net,bullfroggroup.net")).split(",")
    if domain.strip()
]
MEETING_MIN_MINUTES = int(os.getenv("MEETING_MIN_MINUTES", os.getenv("MIN_MEETING_MINUTES", "10")))
MEETING_REQUIRE_EXTERNAL_ATTENDEE = os.getenv("MEETING_REQUIRE_EXTERNAL_ATTENDEE", "true").strip().lower() in {"1", "true", "yes", "on"}
MEETING_INCLUDE_CUSTOM_FIELDS = os.getenv("MEETING_INCLUDE_CUSTOM_FIELDS", "false").strip().lower() in {"1", "true", "yes", "on"}
...
# Optional mapping so Zoho Call Owner is the Webex user who handled the call,
# not the Zoho OAuth/API user that creates the record.
# Render example:
# ZOHO_OWNER_USER_MAP={"michael@bullfroggroup.net":"123456789000001111","zach@bullfroggroup.net":"123456789000002222"}
ZOHO_OWNER_USER_MAP_RAW = os.getenv("ZOHO_OWNER_USER_MAP", "{}").strip() or "{}"
try:
    ZOHO_OWNER_USER_MAP = {
        str(email).strip().lower(): str(user_id).strip()
        for email, user_id in json.loads(ZOHO_OWNER_USER_MAP_RAW).items()
        if str(email).strip() and str(user_id).strip()
    }
except Exception as exc:
    print(f"Invalid ZOHO_OWNER_USER_MAP JSON. Owner mapping disabled: {exc}")
    ZOHO_OWNER_USER_MAP = {}
...
def get_zoho_owner_id_for_call(call_log: Dict[str, Any]) -> Optional[str]:
    """Return the Zoho user ID that should own this call, based on Webex email."""
    email = str(call_log.get("email") or "").strip().lower()
    if not email:
        return None
    return ZOHO_OWNER_USER_MAP.get(email)
...
def build_zoho_call_payload(call_log: Dict[str, Any]) -> Dict[str, Any]:
    ...
    zoho_owner_id = get_zoho_owner_id_for_call(call_log)
    if zoho_owner_id:
        record["Owner"] = {"id": zoho_owner_id}
...
def get_zoho_debug() -> Dict[str, Any]:
    return {
        ...
        "owner_mapping_enabled": bool(ZOHO_OWNER_USER_MAP),
        "owner_mapping_count": len(ZOHO_OWNER_USER_MAP),
...
def is_internal_meeting_email(email: Optional[str]) -> bool:
    if not email or "@" not in str(email):
        return False
    domain = str(email).split("@", 1)[1].strip().lower()
    return domain in MEETING_INTERNAL_DOMAINS


def webex_meeting_get(access_token: str, url: str, params: Optional[Dict[str, Any]] = None) -> requests.Response:
    return webex_request("GET", url, access_token, params=params or {}, timeout=30)



def normalize_meeting_input(raw_value: Optional[str]) -> str:
    """Clean up pasted Webex meeting values.

    Users may paste values like:
      MTID=meabc123
      meetingId=abc123
      https://...?...MTID=meabc123
    This function extracts the likely ID/value before the API lookup.
    """
    value = str(raw_value or "").strip()
    if not value:
        return ""

    # If a URL or query-like string was pasted, pull common parameters first.
    for key in ("meetingId", "meeting_id", "MTID", "mtid", "id"):
        match = re.search(rf"(?:[?&]|^){key}=([^&\s]+)", value)
        if match:
            return match.group(1).strip()

    # If it is just MTID=xxxx.
    if "=" in value:
        left, right = value.split("=", 1)
        if left.strip().lower() in {"mtid", "meetingid", "meeting_id", "id"}:
            return right.strip()

    return value


def find_user_meeting_by_input(access_token: str, raw_value: str) -> Optional[Dict[str, Any]]:
    """Best-effort fallback when /v1/meetings/{id} rejects a pasted value.

    This searches the signed-in user's meeting list and tries to match the pasted
    value against common fields returned by Webex.
    """
    cleaned = normalize_meeting_input(raw_value)
    candidates = {str(raw_value or "").strip(), cleaned}
    candidates = {c for c in candidates if c}

    # Search recent/upcoming meetings visible to this user.
    response = webex_meeting_get(
        access_token,
    ...
    if response.status_code >= 400:
        print(f"Meeting list fallback failed: {response.status_code} {response.text}")
        return None
    ...
    for meeting in items:
        possible_values = {
            ...
            str(meeting.get("meetingId") or ""),
            str(meeting.get("meetingNumber") or ""),
            str(meeting.get("meetingLink") or ""),
            str(meeting.get("sipAddress") or ""),
            str(meeting.get("webLink") or ""),
            str(meeting.get("title") or ""),
        }

        # Exact match or pasted value contained inside a meeting link/string.
        for candidate in candidates:
            if candidate in possible_values:
                return meeting
            if any(candidate and candidate in value for value in possible_values):
                return meeting
...
def list_webex_meetings_for_user(
    access_token: str,
    days_back: int = 30,
    days_forward: int = 7,
    max_results: int = 100,
) -> list:
    """List meetings visible to the signed-in Webex user.

    The ID returned in each meeting object's `id` field is the value that should
    be passed to GET /v1/meetings/{meetingId} and the Meeting Participants API.
    """
    now = datetime.now(timezone.utc)
    date_from = (now - timedelta(days=max(1, days_back))).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    date_to = (now + timedelta(days=max(1, days_forward))).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    ...
    if response.status_code >= 400:
        raise HTTPException(
            ...
            detail=f"Webex List Meetings failed: {response.text}",
        )
    ...
    for meeting in items:
        ...
        normalized.append({
            ...
            "hostEmail": meeting.get("hostEmail") or meeting.get("hostUserEmail"),
            "start": (
    ...
    normalized.sort(
        key=lambda meeting: meeting.get("start") or "",
        reverse=True,
    )
    return normalized


def get_webex_meeting_details(access_token: str, meeting_id: str) -> Dict[str, Any]:
    original_meeting_id = meeting_id
    meeting_id = normalize_meeting_input(meeting_id)

    response = webex_meeting_get(access_token, f"https://webexapis.com/v1/meetings/{meeting_id}")

    if response.status_code < 400:
        return response.json()

    # Some pasted Webex values are MTID=... tokens instead of the API meeting id.
    # If direct lookup fails, search this signed-in user's visible meetings and
    # use the real meeting id returned by Webex.
    fallback_meeting = find_user_meeting_by_input(access_token, original_meeting_id)
    if fallback_meeting:
        real_id = fallback_meeting.get("id") or fallback_meeting.get("meetingId")
        if real_id and real_id != meeting_id:
            response = webex_meeting_get(access_token, f"https://webexapis.com/v1/meetings/{real_id}")
            if response.status_code < 400:
                return response.json()

        # If the list item already has enough details, use it.
        return fallback_meeting

    raise HTTPException(
        status_code=response.status_code,
        detail=(
            "Webex meeting details failed. The value pasted does not look like a valid "
            "Webex Meetings API meeting id for this signed-in user. Try opening the meeting "
            "from Webex and using the meeting id/number from the meeting details, or send from "
            "a meeting-ended webhook record. Webex said: "
            f"{response.text}"
        ),
    )
...
def get_webex_meeting_participants(access_token: str, meeting_id: str) -> list:
    response = webex_meeting_get(
        access_token,
    ...
    if response.status_code >= 400:
        # Keep this non-fatal so hosted meeting details can still be turned into a ticket.
        print(f"Webex meeting participants lookup failed: {response.status_code}: {response.text}")
        return []
    return response.json().get("items", [])



def get_webex_meeting_invitees(access_token: str, meeting_id: str) -> list:
    """Return scheduled invitees when historical participant data is unavailable."""
    response = webex_meeting_get(
        access_token,
    ...
    if response.status_code >= 400:
        print(f"Webex meeting invitees lookup failed: {response.status_code}: {response.text}")
        return []

    return response.json().get("items", [])


def normalize_meeting_invitee(invitee: Dict[str, Any]) -> Dict[str, Optional[str]]:
    email = (
        invitee.get("email")
        or invitee.get("inviteeEmail")
        or invitee.get("personEmail")
    )
    name = (
        invitee.get("displayName")
        or invitee.get("name")
        or invitee.get("inviteeName")
        or email
        or "Unknown Invitee"
    )
    return {
...
def normalize_meeting_participant(participant: Dict[str, Any]) -> Dict[str, Optional[str]]:
    email = (
...
def summarize_webex_meeting_for_user(
    ...
    user_email = str(agent.get("email") or "").strip().lower()
    person_id = str(agent.get("person_id") or "").strip()
    title = meeting.get("title") or meeting.get("topic") or "Webex Meeting"
    host_email = str(meeting.get("hostEmail") or meeting.get("hostUserEmail") or meeting.get("host") or "").strip().lower()

    start_raw = meeting.get("actualStart") or meeting.get("start") or meeting.get("startTime") or meeting.get("scheduledStart")
    end_raw = meeting.get("actualEnd") or meeting.get("end") or meeting.get("endTime") or meeting.get("scheduledEnd")
    ...
    normalized_participants = [normalize_meeting_participant(p) for p in participants]
    normalized_invitees = [normalize_meeting_invitee(i) for i in (invitees or [])]

    attendee_records = normalized_participants if normalized_participants else normalized_invitees
    attendee_source = "participants" if normalized_participants else ("invitees" if normalized_invitees else "none")

    participant_emails = [
        str(p.get("email") or "").strip().lower()
        for p in attendee_records
        if p.get("email")
    ]
    external_participants = [
        p for p in attendee_records
        if p.get("email") and not is_internal_meeting_email(p.get("email"))
    ]

    user_participated = bool(user_email and (user_email == host_email or user_email in participant_emails))
    ...
    return {
        ...
        "participants": attendee_records,
        "actual_participant_count": len(normalized_participants),
        "invitee_count": len(normalized_invitees),
        "participant_count": len(attendee_records),
        "external_count": len(external_participants),
        "attendee_source": attendee_source,
        "attendee_data_available": bool(attendee_records),
        "user_participated": user_participated,
        "has_external": len(external_participants) > 0,
    }


def validate_meeting_can_send(summary: Dict[str, Any], force: bool = False) -> tuple[bool, str]:
    if force:
        return True, "Force send enabled."

    if not summary.get("user_participated"):
        return False, f"{summary.get('user_email') or 'Signed-in user'} was not found as host or participant."

    if int(summary.get("duration_minutes") or 0) < MEETING_MIN_MINUTES:
        return False, f"Meeting was under {MEETING_MIN_MINUTES} minutes."

    if MEETING_REQUIRE_EXTERNAL_ATTENDEE and not summary.get("attendee_data_available"):
        return False, (
            "Webex returned no participant or invitee data for this meeting yet. "
            "Refresh later, or use Force Send if you know it should be logged."
        )

    if MEETING_REQUIRE_EXTERNAL_ATTENDEE and not summary.get("has_external"):
        return False, "Meeting attendee data was found, but no external email addresses were present."

    return True, "Eligible"


def build_revio_ticket_payload_for_meeting(summary: Dict[str, Any]) -> Dict[str, Any]:
    participants = summary.get("participants") or []
    participant_lines = []
    for participant in participants:
        email = participant.get("email") or "no email"
        participant_lines.append(f"- {participant.get('name')} <{email}>")

    participants_text = "\n".join(participant_lines) if participant_lines else "No participants returned by Webex."
    title = summary.get("title") or "Webex Meeting"
    duration_text = f"{int(summary.get('duration_minutes') or 0)} minutes"
    user_email = summary.get("user_email") or "N/A"

    full_details = (
        f"Webex meeting completed.\n\n"
        f"Sent By: {user_email}\n"
        f"Meeting Title: {title}\n"
        f"Meeting ID: {summary.get('meeting_id')}\n"
        f"Start: {summary.get('start_time') or 'N/A'}\n"
        f"End: {summary.get('end_time') or 'N/A'}\n"
    ...
    short_description = f"Webex meeting logged: {title} - {duration_text}"
    if len(short_description) > 145:
        short_description = short_description[:142] + "..."
    ...
    if MEETING_INCLUDE_CUSTOM_FIELDS:
        payload["customFields"] = [
            {"key": "webexMeetingId", "value": summary.get("meeting_id")},
            {"key": "webexMeetingTitle", "value": title},
            {"key": "webexMeetingSentBy", "value": user_email},
            {"key": "webexMeetingHost", "value": summary.get("host_email")},
            {"key": "webexMeetingDurationMinutes", "value": int(summary.get("duration_minutes") or 0)},
            {"key": "webexMeetingParticipantCount", "value": int(summary.get("participant_count") or 0)},
            {"key": "webexMeetingExternalCount", "value": int(summary.get("external_count") or 0)},
        ]
...
def upsert_meeting_ticket_record(
    ...
    with db() as conn:
        conn.execute("""
            INSERT INTO meeting_rev_tickets (
                meeting_id, person_id, user_email, title, host_email, start_time, end_time,
                duration_minutes, external_count, participant_count, psa_ticket_status,
                psa_ticket_id, psa_error, created_at, updated_at, sent_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(meeting_id, person_id) DO UPDATE SET
                ...
                psa_ticket_status = excluded.psa_ticket_status,
                psa_ticket_id = COALESCE(excluded.psa_ticket_id, meeting_rev_tickets.psa_ticket_id),
                psa_error = excluded.psa_error,
        ...
        """, (
            ...
            int(summary.get("duration_minutes") or 0),
            int(summary.get("external_count") or 0),
            int(summary.get("participant_count") or 0),
            status,
            ticket_id,
            error,
            ts,
            ts,
            ts if status == "sent" else None,
        ))


def send_revio_ticket_for_meeting(meeting_id: str, person_id: str, force: bool = False) -> Dict[str, Any]:
    if not revio_is_configured():
        raise HTTPException(
            status_code=500,
            detail=(
    ...
    with db() as conn:
        ...
        existing = conn.execute(
            "SELECT psa_ticket_status, psa_ticket_id FROM meeting_rev_tickets WHERE meeting_id = ? AND person_id = ?",
            (meeting_id, person_id),
        ).fetchone()

    if not agent_row or not is_authenticated_agent_row(agent_row):
        raise HTTPException(status_code=404, detail="Signed-in Webex user was not found. Open /oauth/start again.")

    if existing and existing["psa_ticket_status"] == "sent" and not force:
        return {
            "success": True,
            "status": "duplicate",
            "message": "This meeting was already sent to Rev.io for the signed-in user.",
            "ticket_id": existing["psa_ticket_id"],
        }

    access_token = get_user_token_for_call_control(person_id)
    meeting = get_webex_meeting_details(access_token, meeting_id)
    participants = get_webex_meeting_participants(access_token, meeting_id)
    invitees = []
    if not participants:
        invitees = get_webex_meeting_invitees(access_token, meeting_id)

    summary = summarize_webex_meeting_for_user(
        meeting,
        participants,
        meeting_id,
        dict(agent_row),
    ...
    allowed, reason = validate_meeting_can_send(summary, force=force)
    if not allowed:
        upsert_meeting_ticket_record(summary, "skipped", error=reason)
        return {"success": False, "status": "skipped", "reason": reason, "summary": summary}

    payload = build_revio_ticket_payload_for_meeting(summary)
    url = get_revio_ticket_url()
    headers = revio_headers()
    ...
    if response.status_code in (200, 201, 202):
        ticket_id = extract_ticket_id(response_payload)
        upsert_meeting_ticket_record(summary, "sent", ticket_id=ticket_id)
        return {
            "success": True,
            "status": "sent",
            "ticket_id": ticket_id,
    ...
    error_text = f"{response.status_code}: {response.text[:1000]} | URL: {url}"
    upsert_meeting_ticket_record(summary, "failed", error=error_text)
    raise HTTPException(status_code=502, detail=f"Rev.io meeting ticket creation failed. Rev.io said: {error_text}")


def get_meeting_ticket_records_for_person(person_id: str, limit: int = 25) -> list:
    safe_limit = max(1, min(limit, 100))
    with db() as conn:
        if is_call_log_admin_person(person_id):
            rows = conn.execute("""
                SELECT *
...
def init_postgres_db():
    ...
    with db() as conn:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_meeting_rev_tickets_updated_at ON meeting_rev_tickets (updated_at DESC)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_meeting_rev_tickets_person_id ON meeting_rev_tickets (person_id)")
...
CALL_LOG_ADMIN_EMAILS = {
    email.strip().lower()
    for email in os.getenv("CALL_LOG_ADMIN_EMAILS", os.getenv("ATTENDANT_ADMIN_EMAILS", "")).split(",")
    if email.strip()
}


def get_agent_email_for_person(person_id: Optional[str]) -> str:
    if not person_id:
        return ""
    with db() as conn:
        row = conn.execute(
            "SELECT email FROM agents WHERE person_id = ?",
            (person_id,),
        ).fetchone()
    return str(row["email"] or "").strip().lower() if row else ""


def is_call_log_admin_person(person_id: Optional[str]) -> bool:
    email = get_agent_email_for_person(person_id)
    return bool(email and email in CALL_LOG_ADMIN_EMAILS)


def require_signed_in_person(request: Request) -> str:
    session_id = request.cookies.get("attendant_session")
    person_id = person_id_from_session(session_id)
    if not person_id:
        raise HTTPException(
            status_code=401,
            detail="You are not signed in. Open /oauth/start as your Webex user, then return to the dashboard.",
        )
    return person_id


def require_call_log_access(call_log_id: int, person_id: str) -> Dict[str, Any]:
    with db() as conn:
        row = conn.execute(
            "SELECT id, person_id, email, display_name FROM call_logs WHERE id = ?",
            (call_log_id,),
        ).fetchone()
    ...
    row_dict = dict(row)
    if row_dict.get("person_id") != person_id and not is_call_log_admin_person(person_id):
        raise HTTPException(
            status_code=403,
            detail="You can only view or send PSA/Zoho records for your own completed calls.",
        )
...
def dnd_endpoint_candidates(person_id: str) -> list[str]:
    """Return DND endpoints to try.

    The public Webex Calling user feature endpoint should be tried first. Older
    builds of this app also tried /v1/telephony/config/... as a fallback, but
    Webex can return a confusing internal hydra 404 for that route. Keep the
    fallback disabled by default so users see the real response from the public
    endpoint. Set WEBEX_DND_ENABLE_LEGACY_FALLBACK=true only while testing.
    """
    endpoint = WEBEX_DND_ENDPOINT_TEMPLATE.format(person_id=person_id)
    candidates = [endpoint]

    enable_legacy_fallback = os.getenv("WEBEX_DND_ENABLE_LEGACY_FALLBACK", "false").strip().lower() in {"1", "true", "yes", "on"}
...
def read_person_dnd_settings(person_id: str, org_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    for url in dnd_endpoint_candidates(person_id):
        ...
        errors.append({"endpoint": url, "status_code": response.status_code, "body": response.text[:1200]})

    raise HTTPException(status_code=502, detail=f"Unable to read DND settings from Webex. Attempts: {json.dumps(errors)}")
...
def set_person_dnd_settings(person_id: str, org_id: Optional[str], enabled: bool, ring_reminder: Optional[bool] = None) -> Dict[str, Any]:
    ...
    for url in dnd_endpoint_candidates(person_id):
        ...
        for body in payloads:
            ...
            errors.append({"endpoint": url, "body_sent": body, "status_code": response.status_code, "body": response.text[:1200]})

    raise HTTPException(status_code=502, detail=f"Unable to update DND settings in Webex. Attempts: {json.dumps(errors)}")
...
def health():
    return {
        ...
        "meeting_min_minutes": MEETING_MIN_MINUTES,
        "meeting_require_external_attendee": MEETING_REQUIRE_EXTERNAL_ATTENDEE,
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .meeting-inline-send input[type="text"],
    .meeting-inline-send select {
      min-width: 360px;
      flex: 1 1 360px;
    }
    .meeting-inline-result {
      font-size: 13px;
      margin: 0 0 12px;
      padding: 10px 12px;
      border-radius: 12px;
    ...
    .meeting-inline-result.success {
      display: block;
      background: #dcfce7;
      border-color: #bbf7d0;
      color: #166534;
    }
    .meeting-inline-result.error {
      display: block;
      background: #fee2e2;
      border-color: #fecaca;
      color: #991b1b;
    }
    .meeting-inline-result.info {
      display: block;
      background: #dbeafe;
      border-color: #bfdbfe;
      color: #1d4ed8;
...
<main>
  ...
  <section class="activity-panel">
    <div class="activity-header">
      <h2>Completed Calls & Meetings / PSA + Zoho Log</h2>
      <button class="secondary small" onclick="loadActivityLogs()">Refresh Activity Log</button>
    </div>

    <div class="meeting-inline-send">
      <select id="inlineMeetingId">
        <option value="">Loading your Webex meetings...</option>
      </select>
      <button class="secondary small" onclick="loadAvailableWebexMeetings()">Refresh Meetings</button>
      <label class="activity-meta"><input type="checkbox" id="inlineMeetingForce" /> Force Send</label>
      <button class="small" onclick="sendInlineMeetingToRev()">Send Meeting to Rev</button>
    </div>
    <div id="inlineMeetingResult" class="meeting-inline-result"></div>

    <div id="callLogList" class="activity-list">
      <div class="empty">Loading completed calls and meetings...</div>
    </div>
  </section>
...
async function loadAvailableWebexMeetings() {
  const select = document.getElementById("inlineMeetingId");
  if (!select) return;

  const selectedValue = select.value;
  select.innerHTML = `<option value="">Loading your Webex meetings...</option>`;

  try {
    const res = await fetch(
      "/api/webex-meetings?days_back=30&days_forward=7&limit=100",
      { cache: "no-store", credentials: "same-origin" }
    );
    const data = await res.json().catch(() => ({}));

    if (!res.ok) {
      select.innerHTML = `<option value="">Could not load meetings</option>`;
      setInlineMeetingResult(data.detail || "Webex List Meetings failed.", "error");
      return;
    }

    const meetings = data.meetings || [];
    if (!meetings.length) {
      select.innerHTML = `<option value="">No meetings found for this user</option>`;
      return;
    }

    select.innerHTML =
      `<option value="">Select a Webex meeting...</option>` +
      meetings.map(meeting => {
        const start = meeting.start ? new Date(meeting.start).toLocaleString() : "No start time";
        const number = meeting.meetingNumber ? ` • ${meeting.meetingNumber}` : "";
        const state = meeting.state ? ` • ${meeting.state}` : "";
        const label = `${meeting.title || "Webex Meeting"} • ${start}${number}${state}`;
        return `<option value="${escapeHtml(meeting.id)}">${escapeHtml(label)}</option>`;
      }).join("");
    ...
    console.error(err);
    select.innerHTML = `<option value="">Could not load meetings</option>`;
    setInlineMeetingResult("Could not load Webex meetings. Check Render logs.", "error");
  }
}

async function loadMeetingLogs() {
  try {
    const res = await fetch("/api/meetings?limit=10", { cache: "no-store", credentials: "same-origin" });
    const data = await res.json().catch(() => ({}));

    if (!res.ok) {
      console.warn("Could not load meeting records:", data.detail || data);
      recentMeetings = [];
      renderCallLogs();
      return;
    }
...
async function loadActivityLogs() {
  await Promise.allSettled([loadCallLogs(), loadMeetingLogs()]);
}

function setInlineMeetingResult(message, type = "info") {
  const el = document.getElementById("inlineMeetingResult");
  if (!el) return;
  el.className = `meeting-inline-result ${type}`;
  el.textContent = message || "";
}

async function sendInlineMeetingToRev() {
  const input = document.getElementById("inlineMeetingId");
  const forceBox = document.getElementById("inlineMeetingForce");
  const meetingId = (input?.value || "").trim();
  const force = Boolean(forceBox?.checked);

  if (!meetingId) {
    setInlineMeetingResult("Enter a Webex meeting ID first.", "error");
    return;
  }

  setInlineMeetingResult("Sending meeting to Rev.io PSA...", "info");
  ...
  try {
    const res = await fetch("/api/meetings/send", {
      ...
      headers: { "Content-Type": "application/json" },
      credentials: "same-origin",
      body: JSON.stringify({ meeting_id: meetingId, force })
    });

    const data = await res.json().catch(() => ({}));

    if (!res.ok || data.success === false) {
      const message = data.detail || data.reason || data.message || "Meeting send failed. Check Render logs.";
      setInlineMeetingResult(message, "error");
      await loadMeetingLogs();
      return;
    }

    const ticketText = data.ticket_id ? ` Ticket ID: ${data.ticket_id}.` : "";
    const statusText = data.status ? ` Status: ${data.status}.` : "";
    setInlineMeetingResult(`Meeting processed successfully.${statusText}${ticketText}`, "success");

    if (input) input.value = "";
    await loadMeetingLogs();
  } catch (err) {
    console.error(err);
    setInlineMeetingResult("Meeting request failed before it reached the server.", "error");
  }
}
...
async function loadCallLogs() {
  try {
    const res = await fetch("/api/call-logs?limit=10", { cache: "no-store", credentials: "same-origin" });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      const message = data.detail || "Could not load completed calls.";
      document.getElementById("callLogList").innerHTML = `<div class="empty">${escapeHtml(message)}</div>`;
      return;
    }
...
function renderCallLogs() {
  ...
  const callItems = (recentCallLogs || []).map(log => ({ type: "call", sort_time: log.ended_at || log.created_at || log.updated_at, data: log }));
  const meetingItems = (recentMeetings || []).map(meeting => ({ type: "meeting", sort_time: meeting.sent_at || meeting.updated_at || meeting.created_at || meeting.end_time || meeting.start_time, data: meeting }));
  const combined = [...callItems, ...meetingItems].sort((a, b) => new Date(b.sort_time || 0) - new Date(a.sort_time || 0));

  if (!combined.length) {
    list.innerHTML = `<div class="empty">No completed authenticated calls or meeting records have been logged yet.</div>`;
    return;
  }
  ...
  list.innerHTML = combined.map(item => {
    if (item.type === "meeting") {
      ...
      const status = meeting.psa_ticket_status || "not_sent";
      const disabled = status === "sent";
      const title = meeting.title || "Webex Meeting";
      const user = meeting.user_email || "Signed-in user";
      const duration = `${meeting.duration_minutes || 0} min`;
      const participantText = `${meeting.participant_count || 0} participant${Number(meeting.participant_count || 0) === 1 ? "" : "s"} / ${meeting.external_count || 0} external`;
      const statusLabel = status === "sent"
        ? `PSA Sent${meeting.psa_ticket_id ? ` #${meeting.psa_ticket_id}` : ""}`
        : (status === "failed" ? "PSA Failed" : (status === "skipped" ? "PSA Skipped" : "PSA Not sent"));

      return `
        <div class="call-log-item meeting-log-item">
          <div>
            <span class="activity-type-badge activity-type-meeting">MEETING</span><br>
            ${formatEventTime(meeting.sent_at || meeting.updated_at || meeting.created_at || meeting.end_time || meeting.start_time)}
          </div>
          <div title="${escapeHtml(title)}">
            <strong>${escapeHtml(title)}</strong><br>
            <span class="activity-meta">${escapeHtml(user)}</span>
          </div>
          <div title="${escapeHtml(meeting.meeting_id || "")}">
            Meeting ID<br>
            <span class="activity-meta">${escapeHtml(meeting.meeting_id || "N/A")}</span>
          </div>
          <div><strong>${escapeHtml(duration)}</strong><br><span class="activity-meta">${escapeHtml(participantText)}</span></div>
          <div>
            <div class="action-line">
              <div class="psa-status ${status}">${escapeHtml(statusLabel)}</div>
              <button class="small" ${disabled ? "disabled" : ""} onclick="sendExistingMeetingToRev('${escapeHtml(meeting.meeting_id || "")}')">Send PSA</button>
            </div>
            ${meeting.psa_error ? `<div class="activity-meta" title="${escapeHtml(meeting.psa_error)}">${escapeHtml(meeting.psa_error)}</div>` : ""}
          </div>
        </div>
      `;
    }
    ...
    return `
      <div class="call-log-item">
        <div>
          <span class="activity-type-badge activity-type-call">CALL</span><br>
          ${formatEventTime(log.ended_at)}
        </div>
        <div title="${escapeHtml(user)}"><strong>${escapeHtml(user)}</strong><br><span class="activity-meta">${escapeHtml(log.org_name || "Unknown Org")}</span></div>
        <div title="${escapeHtml(remote)}">${escapeHtml(remote)}<br><span class="direction-badge ${direction}">${escapeHtml(direction)}</span></div>
        ...
        <div>
          ...
          ${status === "failed" && log.psa_error ? `<div class="activity-meta" title="${escapeHtml(log.psa_error)}">${escapeHtml(log.psa_error)}</div>` : ""}

          <div class="action-line zoho-line">
            <div class="psa-status ${zohoStatus}">${escapeHtml(zohoLabel)}</div>
          ...
          ${zohoStatus === "failed" && log.zoho_error ? `<div class="activity-meta" title="${escapeHtml(log.zoho_error)}">${escapeHtml(log.zoho_error)}</div>` : ""}
        </div>
      </div>
    `;
  }).join("");
...
async function sendExistingMeetingToRev(meetingId) {
  if (!meetingId) {
    setInlineMeetingResult("Meeting ID was missing for this row.", "error");
    return;
  }

  const select = document.getElementById("inlineMeetingId");
  if (select) {
    if (![...select.options].some(option => option.value === meetingId)) {
      const option = document.createElement("option");
      option.value = meetingId;
      option.textContent = `Previously logged meeting • ${meetingId}`;
      select.appendChild(option);
    }
    select.value = meetingId;
  }

  await sendInlineMeetingToRev();
}
...
async function sendCallLogToPsa(callLogId, note = "") {
  ...
  try {
    ...
    if (!res.ok || !data.success) {
      ...
      showTransferStatus(message, "error");
      await loadAvailableWebexMeetings();
loadActivityLogs();
      return;
    }
...
def meetings_page(request: Request):
    person_id = person_id_from_session(request.cookies.get("attendant_session"))
    signed_in = bool(person_id)
...
<head>
  ...
  <style>
    ...
    .card {{ background: white; border: 1px solid #d0d5dd; border-radius: 16px; padding: 18px; box-shadow: 0 8px 18px rgba(15,23,42,.06); margin-bottom: 18px; }}
    .row {{ display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }}
    input {{ border: 1px solid #d0d5dd; border-radius: 10px; padding: 11px 12px; font-size: 14px; min-width: 420px; }}
    ...
    table {{ width: max-content; min-width: 1150px; border-collapse: collapse; background: white; }}
    ...
    .not_sent {{ background: #e0f2fe; color: #075985; }}
...
<header>
  ...
  <p>Uses your signed-in Webex token. You can only send meetings your token can read and where your email is host or participant.</p>
</header>
<main>
  <div class="card">
    <h2>Manual Send</h2>
    <p class="muted" id="signinStatus">{'Signed in and ready.' if signed_in else 'You are not signed in. Open /oauth/start first, then return here.'}</p>
    <div class="row">
      ...
      <a class="button secondary" href="/attendantconsole">Back to Console</a>
      <a class="button secondary" href="/oauth/start">Reconnect Webex</a>
    </div>
...
async function loadMeetings() {{
  ...
  if (response.status === 401) {{
    document.getElementById("meetingRows").innerHTML = `<tr><td colspan="9">Not signed in. Open /oauth/start first.</td></tr>`;
    return;
  }}
  ...
  body.innerHTML = rows.map(m => `
    <tr>
      ...
      <td><span class="pill ${{m.psa_ticket_status || "not_sent"}}">${{m.psa_ticket_status || "not_sent"}}</span></td>
      <td>${{m.duration_minutes || 0}} min</td>
      <td>${{m.participant_count || 0}}</td>
      <td>${{m.external_count || 0}}</td>
      <td>${{m.psa_ticket_id || "N/A"}}</td>
      <td>${{m.psa_error || ""}}</td>
      <td><button onclick="sendExisting('${{m.meeting_id}}')">Send</button></td>
    </tr>
  `).join("");
}}

async function sendExisting(meetingId) {{
...
async function sendMeeting() {{
  ...
  if (!meetingId) {{ alert("Enter a Webex meeting ID first."); return; }}
...
@app.get("/api/webex-meetings")
def api_webex_meetings(
    request: Request,
    days_back: int = 30,
    days_forward: int = 7,
    limit: int = 100,
):
    current_person_id = require_signed_in_person(request)
    access_token = get_user_token_for_call_control(current_person_id)

    meetings = list_webex_meetings_for_user(
        access_token=access_token,
        days_back=max(1, min(days_back, 90)),
        days_forward=max(1, min(days_forward, 30)),
...
def api_meetings(request: Request, limit: int = 25):
    current_person_id = require_signed_in_person(request)
    return {"meetings": get_meeting_ticket_records_for_person(current_person_id, limit=limit)}


@app.post("/api/meetings/send")
async def api_send_meeting_to_rev(request: Request):
    current_person_id = require_signed_in_person(request)
    payload = await request.json()
    meeting_id = str(payload.get("meeting_id") or payload.get("meetingId") or "").strip()
    force = bool(payload.get("force"))

    if not meeting_id:
        raise HTTPException(status_code=400, detail="Missing meeting_id")

    result = send_revio_ticket_for_meeting(normalize_meeting_input(meeting_id), current_person_id, force=force)
    return result


@app.get("/api/call-logs")
def api_call_logs(request: Request, limit: int = 10):
    current_person_id = require_signed_in_person(request)
    safe_limit = max(1, min(limit, 50))

    with db() as conn:
        if is_call_log_admin_person(current_person_id):
            rows = conn.execute(
                """
                SELECT *
                FROM call_logs
            ...
            rows = conn.execute(
                ...
                LIMIT ?
                """,
                (current_person_id, safe_limit),
            ).fetchall()

    return {
        "count": len(rows),
        "call_logs": [dict(row) for row in rows],
        "filtered_to_signed_in_user": not is_call_log_admin_person(current_person_id),
    }
...
async def api_send_call_log_to_psa(call_log_id: int, request: Request):
    current_person_id = require_signed_in_person(request)
    require_call_log_access(call_log_id, current_person_id)
